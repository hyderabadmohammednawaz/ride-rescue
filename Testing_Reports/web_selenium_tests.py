...
def generate_web_tests():
    ...
    for i in range(355):
        ...
        test_id = f"WEB_SEL_{category}_{count:03d}"
        test_name = f"Verify web app {category.lower()} scenario {count}"
        test_cases.append({
            "Test_ID": test_id,
...
def run_tests_and_generate_report():
    print("Starting Selenium Web Tests...")
    test_cases = generate_web_tests()
    ...
    ws = wb.active
    ws.title = "Web Selenium Test Report"
    ...
    report_path = os.path.join(os.path.dirname(__file__), 'web_selenium_report.xlsx')
    wb.save(report_path)
